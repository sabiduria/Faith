...
<div class="mt-3">
    ...
    <?= $this->Html->link(__('Nouveau Report'), ['action' => 'add'], ['class' => 'btn btn-success btn-sm mb-3']) ?>
        ...
        <table id="scroll-vertical" class="table table-bordered text-nowrap w-100 TableData">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?= h($report->deleted) ?></td>
                    <td class="actions">
                        <?= $this->Html->link(__('Details'), ['action' => 'view', $report->id], ['class' => 'btn btn-success btn-sm']) ?>
                        <?= $this->Html->link(__('Editer'), ['action' => 'edit', $report->id], ['class' => 'btn btn-primary btn-sm']) ?>
                        <?= $this->Form->postLink(__('Supprimer'), ['action' => 'delete', $report->id], ['class' => 'btn btn-danger btn-sm', 'confirm' => __('Voulez-vous supprimer cette information ?')]) ?>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
...
<div class="offcanvas offcanvas-end" tabindex="-1" id="NewItem"
    ...
    <div class="offcanvas-header border-bottom border-block-end-dashed">
        <h5 class="offcanvas-title" id="offcanvasRightLabel1">Nouveau Reports</h5>
...
<script>
    $(document).ready(function() {
        $('#DataForm').on('submit', function(e) {
            ...
            $.ajax({
                url: '<?= $this->Url->build(["controller" => 'Reports', 'action' => 'insert']) ?>',
                method: 'POST',
                data: formData,
